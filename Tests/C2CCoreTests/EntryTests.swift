...
final class EntryTests: XCTestCase {
    ...
    func testInstallScriptIsIdempotentAndIncludesWorkspaceActions() throws {
        ...
        XCTAssertTrue(source.contains("function detectCodexLanguage()"))
        XCTAssertTrue(source.contains("function refreshLanguage()"))
        XCTAssertTrue(source.contains("action: \"set-locale\""))
        XCTAssertTrue(source.contains("LOCALIZATIONS"))
        ...
        XCTAssertTrue(source.contains("renderedQuickChatConversationIDs"))
        XCTAssertTrue(source.contains("panel.querySelectorAll(\"li button\")"))
        XCTAssertTrue(source.contains("quickChatConversationSnapshot"))
        XCTAssertTrue(source.contains("refreshQuickChatConversationBaseline"))
        XCTAssertTrue(source.contains("stableSnapshotCount >= 12"))
        XCTAssertTrue(source.contains("snapshot.join(\"|\")"))
        XCTAssertTrue(source.contains("await refreshQuickChatConversationBaseline(previousConversationID)"))
        XCTAssertTrue(source.contains("createdIDs.length === 1"))
        XCTAssertTrue(source.contains("Date.now() - quickChatMissingSince >= 10000"))
        ...
        XCTAssertTrue(source.contains("text(\"workspacePath\", { path: sessionMenuWorkspace.path })"))
