...
- 配對與 OAuth 入口具有容量／頻率限制，不以未驗證的 forwarded IP 放寬限制。
...
XCTest 覆蓋配對／OAuth／scope／token 重播、HTTP 解析、真實本機 HTTP 的 OAuth→MCP 流程、工作區與 symlink 邊界、敏感檔案／Git diff、MCP 參數與分頁、執行輸出清洗、會話／偏好／TOML 設定、CLI 本機 setup／復用／status／doctor／stop。

CLI 測試透過 `C2C_STATE_DIR` 與 `C2C_CODEX_CONFIG` 使用臨時目錄；單元測試不需要 ChatGPT 帳號。建置腳本支援 ARM64 與 x86_64 universal binary。
